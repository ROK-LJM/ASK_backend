...
public class BoardController {
    ...
    public MbUserDTO mbLogin(@RequestBody MbUserDTO mbUserDTO){
        // param: mb_id, mb_pw
        System.out.println("================================================");
        System.out.println("[ 모바일 로그인 ] 들어온 mbUserDTO 값: " + mbUserDTO);
        System.out.println("================================================");
        try {
            // 들어온 id, pw 의 계정 존재하는 체크
            int check = boardService.checkMbLogin(mbUserDTO);
            System.out.println("[ 모바일 로그인 ] 결과 check 값 : + " + check);
            if(check == 0){
                return null;
            }

            // 프론트에서 예외처리를 해줘야함(firstCheck 값으로 안내문구를 띄울지 말지 결정하도록 만들어야함)
            // (해당 id와 pw의 정보를 다 가져옴)
            mbUserDTO = boardService.getMbData(mbUserDTO);
            System.out.println("-----[ 모바일 로그인 ] 정상적으로 로그인이 되었습니다-----");
            return mbUserDTO;
        }catch (Exception e){
            System.out.println("-----[에러]모바일 로그인에 실패하였습니다-----");
            e.printStackTrace();
            return null;
        }
    }
    ...
    @RequestMapping("mbFirstUpdate")
    public boolean mbFirstUpdate(@RequestBody MbUserDTO mbUserDTO){
        // param: 세션에 저장된 mb_id, 입력된 mb_pw, 새로 변경할 비밀번호 updatePw
        System.out.println("================================================");
        System.out.println("[ 모바일 로그인 - 최초 로그인(비밀번호 변경) ] 들어온 mbUserDTO 값: " + mbUserDTO);
        System.out.println("================================================");
        try {
            // 비밀번호 변경하기
            int check = boardService.mbFirstUpdate(mbUserDTO);
            System.out.println("[ 모바일 로그인 - 최초 로그인(비밀번호 변경) ] 결과 check 값: " + check);
            if(check == 0){
            ...
            System.out.println("-----[에러]모바일 최초 비밀번호 변경에 실패하였습니다-----");
            e.printStackTrace();
            return false;
        }
    }
    ...
    public boolean createCompany(@RequestBody CompanyDTO companyDTO) {
        ...
        System.out.println("================================================");
        try {
            int checking = boardService.checkCompany(companyDTO);
    ...
    public ArrayList<CompanyDTO> readCompany(@RequestBody CompanyDTO companyDTO) {
        ...
        try {
            ...
            System.out.println("[ 회사설정 - 회사정보 보기 ] 결과값 companyInfo 값 : " + companyInfo);
            if(companyInfo == null){
                return null;
            }
            return companyInfo;
    ...
    public boolean updateCompany(@RequestBody CompanyDTO companyDTO) {
        ...
        try {
            ...
            System.out.println("[  회사설정 - 회사정보 수정 ] 결과값 check 값 : " + check);
            if (check == 1) {
                return true;
            } else {
                return false;
    ...
    public boolean createDep(@RequestBody DepDTO depDTO){
        ...
        try {
            // 중복값 확인
            int depCodeCheck = boardService.getDepCode(depDTO);
            if(depCodeCheck > 0){
                return false;
            }
    ...
    public boolean updateDep(@RequestBody DepDTO depDTO){
        // param data : depId, 모든 데이터
        System.out.println("================================================");
        System.out.println("[ 부서관리 - 부서정보 수정(update) ] 들어온 depDTO 값: " + depDTO);
        System.out.println("================================================");
        try {
            // 중복값 확인
            int depCodeCheck = boardService.getDepCode(depDTO);
            ...
            int check = boardService.updateDep(depDTO);
            System.out.println("[ 부서관리 - 부서정보 수정(update) ] 결과값 check 값 : " + check);
            if(check == 0){
                return false;
            }
    ...
    @RequestMapping("updateDepModal")
    public ArrayList<DepDTO> updateDepModal(@RequestBody DepDTO depDTO){
        // param data : depId
        System.out.println("================================================");
        System.out.println("[ 부서관리 - 부서정보 수정 모달(modal) ] 들어온 depDTO 값: " + depDTO);
        System.out.println("================================================");
        try {
            ArrayList<DepDTO> depList = boardService.updateDepModal(depDTO);
            System.out.println("[ 부서관리 - 부서정보 수정 모달(modal) ] 결과값 depList 값 : " + depList);
            if(depList == null){
                return null;
            }
            return depList;
    ...
    public boolean createEmp(@RequestBody EmpDTO empDTO){
        ...
        empDTO.setEmpSSN(empDTO.getEmpFirstSSN().trim() + "-" + empDTO.getEmpSecondSSN().trim());
        System.out.println("완성된 주민등록번호: " + empDTO.getEmpSSN());

        // [입사일] 년/월/일 합치기
        empDTO.setEmpStart(empDTO.getEmpStartYear().trim() + "-" + empDTO.getEmpStartMonth().trim() + "-" + empDTO.getEmpStartDay().trim());
        ...
        if(empDTO.getEmpEndYear() != null && empDTO.getEmpEndMonth() != null && empDTO.getEmpEndDay() != null){
            empDTO.setEmpEnd(empDTO.getEmpEndYear().trim() + "-" + empDTO.getEmpEndMonth().trim() + "-" + empDTO.getEmpEndDay().trim());
            System.out.println("완성된 퇴사일: " + empDTO.getEmpEnd());
        }

        // 잔여 휴가 값 넣기
        empDTO.setRemindVacation(empDTO.getTotalVacation());
        System.out.println("총 휴가 : " + empDTO.getTotalVacation() + " " + "잔여휴가 : " + empDTO.getRemindVacation());

        try {
            // 중복값 확인
            int empNumCheck = boardService.getEmpCode(empDTO);
            if(empNumCheck > 0){
                return false;
            }
    ...
    public boolean createEmpPay(@RequestBody Emp_payDTO emp_payDTO){
        ...
        System.out.println("================================================");
        try {
            // 중복값 확인(empPayID, payCode)
            int payCodeCheck = boardService.getPayCode(emp_payDTO);
            if(payCodeCheck > 0){
                return false;
            }

            int check = boardService.createEmpPay(emp_payDTO);
    ...
    public boolean updateCalc(@RequestBody Emp_payDTO emp_payDTO){
        // param : taxFreeCalc, empPayID
        System.out.println("================================================");
        System.out.println("[ 수당관리 - 수당 계산식 update ] 들어온 emp_payDTO 값: " + emp_payDTO);
        System.out.println("================================================");
        try {
            int updateCalc = boardService.updateCalc(emp_payDTO);
            System.out.println("[ 수당관리 - 수당 계산식 update ] 결과값 updateCalc 값: " + updateCalc);
            if(updateCalc == 0){
                return false;
            }
            return true;
    ...
    public boolean deleteEmpPay(@RequestBody Emp_payDTO emp_payDTO){
        ...
        String[] deleteArr;
        try {
            int check = boardService.deleteEmpPay(emp_payDTO);
    ...
    @RequestMapping("readTaxFree")
    public ArrayList<TaxFreeDTO> readTaxFree(@RequestBody TaxFreeDTO taxFreeDTO){
        System.out.println("==================================");
        System.out.println("[ 수당관리 - 비과세 목록 가져오기(read) ] 들어온 taxFreeDTO: " + taxFreeDTO);
        System.out.println("==================================");
        try {
            if(taxFreeDTO.getCheck() != 1){
                return null;
            }
            ArrayList<TaxFreeDTO> taxFreeList = boardService.readTaxFree(taxFreeDTO);
            System.out.println("[ 수당관리 - 비과세 목록 가져오기(read) ] 결과값 check : " + taxFreeList);
            if(taxFreeList == null){
                return null;
            }
            return taxFreeList;
    ...
    public boolean createTax(@RequestBody TaxDTO taxDTO){
        // param : 모든 데이터, compCode
        System.out.println("==================================");
        System.out.println("[ 세금관리 - 세금정보 등록(create) ] 들어온 taxDTO: " + taxDTO);
        System.out.println("==================================");
        try {
            // 중복값 확인
            int taxCodeCheck = boardService.checkCreateTax(taxDTO);
            if(taxCodeCheck > 0){
                System.out.println("--- 해당 세금코드는 이미 존재합니다. ---");
                return false;
            }

            int check = boardService.createTax(taxDTO);
            System.out.println("등록된 taxDTO 값 : " + taxDTO);
    ...
    // 세금관리 - 기본 세금항목들 불러오기(read)
    @RequestMapping("readTaxCategory")
    public ArrayList<TaxCategoryDTO> readTaxCategory(@RequestBody TaxCategoryDTO taxCategoryDTO){
        System.out.println("================================================");
        System.out.println("[ 세금관리 - 기본 세금항목들 불러오기(read) ] 들어온 taxCategoryDTO 값: " + taxCategoryDTO);
        System.out.println("================================================");
        try {
            ArrayList<TaxCategoryDTO> taxCategoryList = boardService.readTaxCategory(taxCategoryDTO);
            System.out.println("[ 세금관리 - 기본 세금항목들 불러오기(read) ] 결과값 taxList 값 : " + taxCategoryList);
    ...
    @RequestMapping("getTaxCategory")
    public ArrayList<TaxCategoryDTO> getTaxCategory(@RequestBody TaxCategoryDTO taxCategoryDTO){
        System.out.println("================================================");
        System.out.println("[ 세금관리 - 선택한 세금항목 값 주기(read) ] 들어온 taxCategoryDTO 값: " + taxCategoryDTO);
        System.out.println("================================================");
        try {
            ArrayList<TaxCategoryDTO> taxCategoryList = boardService.getTaxCategory(taxCategoryDTO);
            System.out.println("[ 세금관리 - 선택한 세금항목 값 주기(read) ] 결과값 taxList 값 : " + taxCategoryList);
            if(taxCategoryList == null){
                return null;
            }
            return taxCategoryList;
    ...
    public boolean updateTax(@RequestBody TaxDTO taxDTO){
        ...
        try {
            // 중복값 확인
            int taxCodeCheck = boardService.getTaxCode(taxDTO);
            if(taxCodeCheck > 0){
                return false;
            }
    ...
    public boolean deleteTax(@RequestBody TaxDTO taxDTO){
        ...
        System.out.println("================================================");
        try {
            int check = boardService.deleteTax(taxDTO);
            System.out.println("[ 수당관리 - 수당항목 삭제(delete) ] 결과값 check 값: " + check);
            if(check == 0){
                return false;
            }
...
//            expense_infoDTO.setTotalPrice(expense_infoDTO.getPrice());  //금액 total에 넣어줌
    ...
    //------------------------------- < 모바일에서 영수증 이미지 s3에 업로드 > ---------------------------------------
    // [모바일]영수증 이미지 저장(영수증 이미지 s3에 업로드)
    @RequestMapping("uploadImage")
    public boolean uploadImage(@RequestPart("imageData") MultipartFile multipartFile, @RequestPart("jsonList") Expense_infoDTO expense_infoDTO) throws IOException {
        System.out.println("==================================");
        System.out.println("[ [모바일]영수증 이미지 저장 ] multipartFile 값 : " + multipartFile);
        System.out.println("[ [모바일]영수증 이미지 저장 ] expense_infoDTO 값 : " + expense_infoDTO);
        System.out.println("==================================");


        try {
            // 해당 회사가 가지고 있는 영수증 갯수 최대치 값 가져오기(쉽게 카운팅하기 위함)
            int cnt = boardService.getCountReceipt(expense_infoDTO);
            cnt++;

            // s3에 파일 업로드 및 url값 세팅하기
            // return 값으로 s3에 저장한 url값이 포함된 expense_infoDTO값을 반환받는다.
            expense_infoDTO = s3UploaderService.upload(expense_infoDTO, multipartFile, "receipt", cnt);
            System.out.println("파일 변환 후, 결과값 expense_infoDTO : " + expense_infoDTO);
            if(expense_infoDTO == null){
                System.out.println("----- [null]파일 변환 후, 결과에 문제가 생겼습니다 -----");
                return false;
            }

            // 세팅된 expense_infoDTO값을 create한다.
            int check = boardService.expenseCreate(expense_infoDTO);
            System.out.println("[ [모바일]영수증 이미지 저장 ] 결과값 check 값 : " + check);
            if(check == 0){
                System.out.println("----- [check==0] 경비 추가를 실패하였습니다 -----");
                return false;
            }

            return true;
        }catch (Exception e){
            System.out.println("----- [에러]영수증 업로드를 실패하였습니다 -----");
            e.printStackTrace();
            return false;
        }
    }
    ...
    // [모바일]영수증 이미지 저장(영수증 이미지 s3에 업로드) - [테스트용 이미지 업로드 코드]
    /*
    @RequestMapping("uploadImage2")
    public Expense_infoDTO uploadImage2(@RequestParam("imageData") MultipartFile multipartFile) throws IOException {
        Expense_infoDTO expense_infoDTO = new Expense_infoDTO();
        expense_infoDTO.setCompCode("admin01");
        expense_infoDTO.setEmpNum("008");
        expense_infoDTO.setEmpName("최범근");
        expense_infoDTO.setDepCode("001");
        expense_infoDTO.setDepName("개발부");
        expense_infoDTO.setShopName("테스트 상점");
        expense_infoDTO.setShopNum("2714500582");
        expense_infoDTO.setExpenseDate("20221204");
        expense_infoDTO.setPrice(15000);
        expense_infoDTO.setApproval("요청");
        expense_infoDTO.setRequestDate("20221204");
        System.out.println("==================================");
        System.out.println("[ [모바일]영수증 이미지 저장 ] multipartFile 값 : " + multipartFile);
        System.out.println("[ [모바일]영수증 이미지 저장 ] expense_infoDTO 값 : " + expense_infoDTO);
        System.out.println("==================================");
        ...
        try {
            // 해당 회사가 가지고 있는 영수증 갯수 최대치 값 가져오기(쉽게 카운팅하기 위함)
            int cnt = boardService.getCount(expense_infoDTO);
            cnt++;
            System.out.println("cnt + 1 값 : " + cnt);
    ...
    */
    ...
    public List<Expense_infoDTO> expenseRead(@RequestBody Expense_infoDTO expense_infoDTO){
        ...
        try {
            ...
            System.out.println("[ [경비관리 read ] 결과값 list 값 : " + list);
    ...
    public boolean expenseUpdate(@RequestBody Expense_infoDTO expense_infoDTO){
        ...
        try {
            ...
            // 만약 요청승인여부로 "요청"이 들어왔다면..(요청날짜 입력)
            if(expense_infoDTO.getApproval().equals("요청")){
                // 포맷된 현재날짜 적용
                expense_infoDTO.setRequestDate(formatedNow);
                check = boardService.expenseUpdateRequest(expense_infoDTO);
            }
            // 포맷된 현재날짜 적용
            expense_infoDTO.setApprovalDate(formatedNow);
            // 그 밖에 "승인", "미승인"이 들어왔다면..(승인 혹은 미승인 날짜 입력)
            check = boardService.expenseUpdateApproval(expense_infoDTO);
    ...
    public boolean expenseDelete(@RequestBody Expense_infoDTO expense_infoDTO){
        ...
        try {
            // [ 삭제하려는 정보 가져오기 ]
            List<Expense_infoDTO> list = boardService.expenseModal(expense_infoDTO);
            if(list == null){
                System.out.println("-----<삭제하려는 정보를 가져오지 못했습니다>-----");
                return false;
            }

            // [ 삭제할 영수증 s3 파일이름 정제하기 ]
            ...
            String fileName = list.get(0).getDocumentation();
            if(fileName != null){
                int beginIndex = fileName.indexOf("com") + 4;           // /receipt 에서 r부터 나오도록 인덱스 설정
                int endIndex = fileName.length();                       // url 끝 지점까지 인덱스 설정
                fileName = fileName.substring(beginIndex, endIndex);    // 문자열 자르기
                System.out.println("파일 이름 정제 결과 : " + fileName);

                // [ 영수증 정보 - s3에서 삭제 ]
                s3UploaderService.deleteS3(fileName);
            }

            // DB에서도 삭제
            int check = boardService.expenseDelete(expense_infoDTO);
            System.out.println("[ 경비관리  삭제 들어온] ] 결과값 check 값 : " + check);
            if(check == 0){
                System.out.println("----- [chekc==0]경비관리 삭제에 실패하였습니다 -----");
                return false;
            }

            return true;
        }catch (Exception e){
            System.out.println("----- [에러]경비관리 삭제에 실패하였습니다 -----");
            e.printStackTrace();
            return false;
        }
    }
    ...
    public List<Expense_infoDTO> expenseModal(@RequestBody Expense_infoDTO expense_infoDTO){
        ...
        try {
            List<Expense_infoDTO> list = boardService.expenseModal(expense_infoDTO);
            System.out.println("[ 경비관리[모달창]   ] 결과값 list 값 : " + list);
            if(list == null){
                return null;
            }
            return list;
        ...
        try{
            ...
            if(expense_infoDTO.getStartDate() == null && expense_infoDTO.getEndDate() == null && expense_infoDTO.getEmpName() != null){
                ...
                for(int i=0; i<searchName.size(); i++){
                    totalPrice += searchName.get(i).getPrice();
                }
                for(int i=0; i<searchName.size(); i++){
                    searchName.get(i).setTotalPrice(totalPrice);
                }
            ...
            if(expense_infoDTO.getEmpName() == null && expense_infoDTO.getStartDate() != null && expense_infoDTO.getEndDate() != null){
                ...
                for(int i=0; i<searchDate.size(); i++){
                    totalPrice += searchDate.get(i).getPrice();
                }
                for(int i=0; i<searchDate.size(); i++){
                    searchDate.get(i).setTotalPrice(totalPrice);
                }
            ...
            if (expense_infoDTO.getStartDate() != null && expense_infoDTO.getEmpName() != null && expense_infoDTO.getEndDate() == null){
                ...
                // totalPrice 값 세팅하기
                for(int i=0; i<searchstartDate.size(); i++){
                    totalPrice += searchstartDate.get(i).getPrice();
                }
                for(int i=0; i<searchstartDate.size(); i++){
                    searchstartDate.get(i).setTotalPrice(totalPrice);
                }
            ...
            if(expense_infoDTO.getStartDate() != null && expense_infoDTO.getEndDate() == null && expense_infoDTO.getEmpName() == null){
                ...
                for(int i=0; i<checklast.size(); i++){
                    totalPrice += checklast.get(i).getPrice();
                }
                for(int i=0; i<checklast.size(); i++){
                    checklast.get(i).setTotalPrice(totalPrice);
                }
    ...
    // ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ < 경리/회계 > - 거래처관리 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
    //거래처 관리 (create)
    ...
    public boolean clientCreate(@RequestBody Client_infoDTO client_infoDTO){
        ...
        try {
            ...
            System.out.println("[거레처관리] 결과 값 : " + check);
            if(check == 0){
                return false;
            }
            return true;
    ...
    public boolean clientDelete(@RequestBody Client_infoDTO client_infoDTO){
        System.out.println("==================================");
        System.out.println("[거래처관리 삭제 들어온 client_infoDTO 값 : " + client_infoDTO);
        System.out.println("==================================");
        try {
            int check = boardService.clientDelete(client_infoDTO);
            System.out.println("[ 거래처관리  삭제 ] 결과값 check 값 : " + check);
            if(check == 0){
                return false;
            }
            return true;
    ...
    public List<Client_infoDTO> clientModal(@RequestBody Client_infoDTO client_infoDTO){
        ...
        try {
            ...
            System.out.println("[ 거래처관리 모달창 ] list 결과값 : " + list);
            if(list == null){
                return null;
            }
            return list;
    ...
    // ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ < 경리/회계 > - 매입관리 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
    //매입관리 (create)
...
//            incomeDTO.setTaxBill(filePath);
    ...
    //------------------------------------------< 매입, 매출 차트 >-------------------------------------------------------
    // 1년치 매입, 매출 정보값 프론트에 전달할 것

    // 매입
    @RequestMapping("readIncomeChart")
    public ArrayList<Integer> readIncomeChart(@RequestBody IncomeDTO incomeDTO){
        // param : compCode
        // p_division: 구분 -> 구분 값으로 매입, 매출 정보값 들어가는게 달라짐
        // 1년치 데이터 보여주기(월 마다의 매입, 매출 총합 값 이용)
        // compCode, startDate, endDate
        System.out.println("==================================");
        System.out.println("[매입] 들어온 CompCode 값 : " + incomeDTO.getCompCode());
        System.out.println("==================================");
        try {
            ArrayList<Integer> incomeResultList = new ArrayList<>();
            ...
            for(int i=1; i<13; i++){
                ...
                if(i<10){
                    month = "0" + month;    // 01, 02,..., 09
                }
                // date 세팅
                date = year + month;    // 202201
                System.out.println("날짜값: " + date);

                // 시작날짜 세팅
                incomeDTO.setStartDate(date+"01");

                // 끝날짜 세팅(30일: 2, 4, 6, 9, 11 / 31일: 1, 3, 5, 7, 8, 10, 12)
                switch (i){
                    case 2:
                        ...
                        if(tempYear % 4 == 0 && tempYear % 100 != 0 || tempYear % 400 == 0){
                            // 윤년
                            incomeDTO.setEndDate(date+"29");
                            resultData = boardService.getIncomeData(incomeDTO);
                            if(resultData == null){
                                resultData = 0;
                            }
                            System.out.println("결과:" + resultData);

                            incomeResultList.add(resultData);
                        }else{
                            // 평년
                            incomeDTO.setEndDate(date+"28");
                            resultData = boardService.getIncomeData(incomeDTO);
                            if(resultData == null){
                                resultData = 0;
                            }
            ...
            System.out.println("[매입,매출 차트] 결과값 incomeResultList: " + incomeResultList);
            if(incomeResultList == null){
                System.out.println("----- [null]매입 데이터 불러오기 실패하였습니다 -----");
                return null;
            }

            return incomeResultList;
        }catch (Exception e){
            System.out.println("----- [에러]매입 데이터 불러오기 실패하였습니다 -----");
            e.printStackTrace();
            return null;
        }
    }
    ...
    // 매출
    @RequestMapping("readOutcomeChart")
    public ArrayList<Integer> readOutcomeChart(@RequestBody IncomeDTO incomeDTO){
        // param : compCode
        // compCode, startDate, endDate
        try {
            ArrayList<Integer> outcomeResultList = new ArrayList<>();
            Integer resultData = 0;
                ...
                switch (i){
                    case 2:
                        ...
                        if(tempYear % 4 == 0 && tempYear % 100 != 0 || tempYear % 400 == 0){
                            // 윤년
                            incomeDTO.setEndDate(date+"29");
                            resultData = boardService.getOutcomeData(incomeDTO);
                            if(resultData == null){
                                resultData = 0;
                            }

                            outcomeResultList.add(resultData);
                        }else{
                            // 평년
                            incomeDTO.setEndDate(date+"28");
                            resultData = boardService.getOutcomeData(incomeDTO);
            ...
            System.out.println("[매입,매출 차트] 결과값 outcomeResultList: " + outcomeResultList);
            if(outcomeResultList == null){
                System.out.println("----- [null]매출 데이터 불러오기 실패하였습니다 -----");
                return null;
            }

            return outcomeResultList;
        }catch (Exception e){
            System.out.println("----- [에러]매출 데이터 불러오기 실패하였습니다 -----");
            e.printStackTrace();
            return null;
        }
    }
    ...
    //------------------------------------------< 모바일 메인페이지 >------------------------------------------------------
    // 로그인한 사원의 출퇴근 값 불러오기
    @RequestMapping("mbMainPage")
    public InOut_infoDTO mbMainPage(@RequestBody InOut_infoDTO inOut_infoDTO){
        // param: compCode=회사코드, empCode=사원코드
        System.out.println("==================================");
        System.out.println("[ 로그인한 사원의 출퇴근 값 불러오기[임직원] ] 들어온 inOut_infoDTO: " + inOut_infoDTO);
        System.out.println("==================================");
        try {
            inOut_infoDTO = boardService.getInOutData(inOut_infoDTO);
            System.out.println("[ 로그인한 사원의 출퇴근 값 불러오기[임직원] ] 결과값 : " + inOut_infoDTO);
            if(inOut_infoDTO == null){
                System.out.println("-----[null]로그인한 사원의 출퇴근 값을 불러오지 못했습니다-----");
                return null;
            }
            System.out.println("출근시간 : " + inOut_infoDTO.getInOutStart());
            System.out.println("퇴근시간 : " + inOut_infoDTO.getInOutEnd());

            return inOut_infoDTO;
        }catch (Exception e){
            System.out.println("-----[에러]로그인한 사원의 출퇴근 값을 불러오지 못했습니다-----");
            e.printStackTrace();
            return null;
        }
    }
