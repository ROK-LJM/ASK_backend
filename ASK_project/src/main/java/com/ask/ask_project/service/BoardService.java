...
public interface BoardService {
    ...
    //보유휴가현황 모달창
    List<Vact_disposeDTO> modalVactlist(Vact_disposeDTO vact_disposeDTO) throws Exception;
    //보유휴가 현황 (update)
    ...
    //출퇴근관리 야간근무시간 업데이트
    int OvertimeNightUpdate(InOut_infoDTO inOut_infoDTO) throws Exception;
    // 출퇴근정보 가져오기
    List<InOut_infoDTO> getInOut_infoTime(InOut_infoDTO inOut_infoDTO) throws Exception;
...
//    // 급여관리 - 사원정보 불러오기(read)
//    EmpDTO readEmpSalay(EmpDTO empDTO) throws Exception;
    ...
    // 급여관리 - 사원정보 불러오기(read)
    EmpDTO readEmpSalay2(EmpDTO empDTO) throws Exception;
    ...
    //경비관리 update
    int expenseUpdate(Expense_infoDTO expense_infoDTO) throws Exception;
    //경비관리 delete
    ...
    //주소 -> 위경도 변환
    HashMap<String, String> getAddress(String address) throws Exception;
